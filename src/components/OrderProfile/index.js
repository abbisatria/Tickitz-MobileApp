  ...
  render() {
    ...
    return (
      <View style={styles.scene}>
        {this.props.order.orderHistory !== null ? (<FlatList
          showsVerticalScrollIndicator={false}
          data = {this.props.order.orderHistory}
          keyExtractor = { item => String(item.id) } 
          renderItem = {({ item }) => (
            <View style={styles.card}>
              <Image source={{uri: `${API_URL}uploads/cinemas/${item.image}`}} style={styles.imageCinema} />
              <Text style={styles.date}>{moment(item.createdAt).format('dddd, D MMMM YYYY - hh:mm A')}</Text>
